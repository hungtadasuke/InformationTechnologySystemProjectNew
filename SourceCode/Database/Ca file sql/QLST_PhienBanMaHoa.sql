create database SuperMarket_Management_DB_Cryptography;

go

use SuperMarket_Management_DB_Cryptography;
...
(
	...
	ImportPrice text null,
	PriceToSell text null,
...
as
begin
	select C.CategoryName, P.ProductId,  P.ProductName, P.Unit, P.ImportPrice, P.PriceToSell, P.Quantity, P.ProductImg, P.StatusItem, P.Barcode from Product as P, Category as C where P.CategoryId = C.CategoryId; 
end
...
create procedure insertProduct @ProductImg image, @ProductName nvarchar(100), @ImportPrice text, @PriceToSell text, @Unit nvarchar(20), @Quantity int, @StatusItem bit, @CategoryId varchar(4), @BarCode varchar(13)
as
begin
	if(@BarCode = 'null')
		insert into Product values (@ProductImg, @ProductName, @ImportPrice, @PriceToSell, @Unit, @Quantity, @StatusItem, @CategoryId, NULL);
	else
		insert into Product values (@ProductImg, @ProductName, @ImportPrice, @PriceToSell, @Unit, @Quantity, @StatusItem, @CategoryId, @BarCode);
end
...
create procedure updateProduct @ProductImg image, @ProductName nvarchar(100), @ImportPrice text, @PriceToSell text, @Unit nvarchar(20), @Quantity int, @StatusItem bit, @CategoryId varchar(4), @ProductId varchar(6), @BarCode varchar(13)
as
begin
	if(@BarCode = 'null')
		update Product set ProductName = @ProductName, ProductImg = @ProductImg, ImportPrice = @ImportPrice, PriceToSell = @PriceToSell, Unit = @Unit, Quantity = @Quantity, StatusItem = @StatusItem, CategoryId = @CategoryId, Barcode = NULL where ProductId = @ProductId;
	else
		update Product set ProductName = @ProductName, ProductImg = @ProductImg, ImportPrice = @ImportPrice, PriceToSell = @PriceToSell, Unit = @Unit, Quantity = @Quantity, StatusItem = @StatusItem, CategoryId = @CategoryId, Barcode = @BarCode where ProductId = @ProductId;
end
